...
def encodeByte(byte, n, k):
    ...
    for j in xrange(0, n):
        ...
        # If we do not do that then the output keys will NEVER have 00 in even positions (starting at 0)
        # which would be a little suspicious for some random data
        #
        ...
        while picked[pick] or pick == 0:
            ...
            if pick == 0:
                keys[j] += chr(0)
                keys[j] += chr(random.randint(0, 255))
...
def main():
    input_string_io = StringIO("Too many secrets, Marty!")
    outputs = []
    n = 5
    k = 3
    for i in xrange(n):
    ...
    encode(input_string_io, outputs, k)
